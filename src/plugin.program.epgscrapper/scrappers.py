...
def getEpgYes(id, days):
	...
	for day in range(0, days):
		...
		for tag in content.find_all(class_="text"):	
			...
			start = datetime.datetime(year=theday.year, month=theday.month, day=theday.day, hour=int(pdata[0].split(':')[0]), minute=int(pdata[0].split(':')[1]), tzinfo=util.timezone(tzOffset))
			programs.append({
				'start': start, 
				'title': pdata[1],
				'description': ''
		...
		programs[-1]['stop'] = datetime.datetime(year=theday.year, month=theday.month, day=theday.day, hour=23, minute=59, tzinfo=util.timezone(tzOffset))
...
def getEpgYandex(id, days):
	...
	for day in range(0, days):
		...
		for pdata in data:	
			start = datetime.datetime(year=int(pdata['start'][0:4]), month=int(pdata['start'][5:7]), day=int(pdata['start'][8:10]), hour=int(pdata['start'][11:13]), minute=int(pdata['start'][14:16]), tzinfo=util.timezone(tzOffset))
			stop = datetime.datetime(year=int(pdata['finish'][0:4]), month=int(pdata['finish'][5:7]), day=int(pdata['finish'][8:10]), hour=int(pdata['finish'][11:13]), minute=int(pdata['finish'][14:16]), tzinfo=util.timezone(tzOffset))
			program = {
				'start': start,
				'stop': stop,
